  ...
  return (
    <div className="card mt-4">
      ...
      <div className="flex justify-between mb-1">
        <span className="font-mono text-[10px] uppercase tracking-widest text-on-surface-dim">Margem</span>
        <span className="qtm-num text-xs font-bold text-on-surface">{margem}%</span>
      </div>
      <input
      ...
      <div className="flex justify-between mb-1 mt-3">
        <span className="font-mono text-[10px] uppercase tracking-widest text-on-surface-dim">Taxas (plataforma+cartão+imposto)</span>
        <span className="qtm-num text-xs font-bold text-on-surface">{taxas}%</span>
      </div>
      <input
        ...
        className="w-full accent-primary"
      />
      <div className="flex justify-between items-end mt-4 pt-3 border-t border-outline">
        <div>
          <p className="label">Custo</p>
          <p className="qtm-num text-sm text-on-surface">{brl(custo)}</p>
        </div>
        <div className="text-center">
          <p className="label">Preço de venda</p>
          {preco ? (
            <p className="qtm-num text-lg font-bold text-on-primary bg-primary px-2 rounded-lg">{brl(preco)}</p>
          ) : (
            <p className="font-sans text-xs text-danger">Margem + taxas ≥ 100%</p>
          )}
        </div>
        <div className="text-right">
          <p className="label">Lucro</p>
          <p className="qtm-num text-sm text-primary">{lucro ? brl(lucro) : '—'}</p>
        </div>
      </div>
    </div>
  )
