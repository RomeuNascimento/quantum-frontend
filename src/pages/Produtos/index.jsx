    ...
    <Layout title="Produtos">
      <div className="px-4 pt-4">
        <div className="flex justify-end mb-4">
          <Link to="/produtos/novo" className="bg-primary-600 text-white text-sm font-semibold px-4 py-2 rounded-xl">
            + Novo
          </Link>
        </div>
            ...
            {items.map((p) => (
              <div key={p.id} className="card flex items-center justify-between gap-3">
                <Link to={`/produtos/${p.id}`} className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500">Ver custo e precificação →</p>
                </Link>
                <button onClick={() => handleDelete(p.id, p.nome)} className="p-2 text-gray-400 active:text-red-500">
